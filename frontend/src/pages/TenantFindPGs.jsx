  ...
  const handleApply = async (pg) => {
    ...
    try {
      ...
      if (available.length === 0) {
        toast.warning("No rooms available for this PG right now.");
        return;
      }
            ...
            <div className="find-layout">
              ...
              <div className="filter-panel">
                ...
                <div className="filter-group">
                  ...
                  <input className="clay-input" type="text" placeholder="Enter city or area" value={filters.location} onChange={(e) => setFilters({ ...filters, location: e.target.value })} />
                </div>
                <div className="filter-group">
                  <label className="clay-label">Budget Range (₹)</label>
                  <div className="budget-row">
                    <input className="clay-input" type="number" placeholder="Min" value={filters.budgetMin} onChange={(e) => setFilters({ ...filters, budgetMin: e.target.value })} />
                    <input className="clay-input" type="number" placeholder="Max" value={filters.budgetMax} onChange={(e) => setFilters({ ...filters, budgetMax: e.target.value })} />
                  </div>
                </div>
                <div className="filter-group">
                  ...
                  {["Single", "Double", "Triple"].map((type) => (
                    <div key={type} className="clay-checkbox-row">
                      <input type="checkbox" id={`rt-${type}`} checked={filters.roomType.includes(type)} onChange={() => toggleFilter("roomType", type)} />
                ...
                <div className="filter-group">
                  ...
                  {["WiFi", "AC", "Meals", "Laundry"].map((a) => (
                    <div key={a} className="clay-checkbox-row">
                      <input type="checkbox" id={`am-${a}`} checked={filters.amenities.includes(a)} onChange={() => toggleFilter("amenities", a)} />
                  ...
                  pgListings.map((pg, i) => (
                    <div key={pg._id} className="pg-card" style={{ animationDelay: `${i * 0.07}s` }}>
                      <div className="pg-card-inner">
                        ...
                        <button className="apply-btn" onClick={() => handleApply(pg)} disabled={applying === pg._id}>
